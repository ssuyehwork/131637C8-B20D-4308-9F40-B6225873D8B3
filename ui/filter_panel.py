...
from PyQt5.QtGui import QDrag, QPixmap, QPainter, QCursor, QColor, QPen, QIcon
...
class FilterPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.resize_handle = QLabel(self.container)
        ...
        header_icons = {
            'stars': ('star_filled.svg', '#f39c12'),
            'colors': ('palette.svg', '#e91e63'),
            'types': ('branch.svg', '#3498db'), 
            'date_create': ('calendar.svg', '#2ecc71'),
            'tags': ('tag.svg', '#e67e22')
        }
    ...
    def update_stats(self, stats):
        ...
        for c_hex, count in stats['colors'].items():
            if count > 0: color_data.append((c_hex, c_hex, count)) 
        ...
        # 5. [还原] 类型 (使用 item_type 值)
        type_data = []
        # 将 item_type 映射为显示名称
        type_map = {'text': '文本', 'image': '图片', 'file': '文件', 'folder': '文件夹'}
        
        for t, count in stats.get('types', {}).items():
            if count > 0:
                # 简单映射图标，实际项目可根据 ext 返回不同 icon
                label = type_map.get(t, t.upper())
                type_data.append((t, label, count))
        
        # 按数量倒序排列
        type_data.sort(key=lambda x: x[2], reverse=True)
    ...
    def _refresh_node(self, key, data_list, is_col=False):
        ...
        for data_item in data_list:
            ...
            if item_key in existing_items:
                ...
                if child.text(0) != new_text: child.setText(0, new_text)
                if icon: child.setIcon(0, icon)
                elif not is_col and not icon: child.setIcon(0, QIcon())
            else:
                child = QTreeWidgetItem(root)
                ...
                child.setCheckState(0, Qt.Unchecked)
                if icon: child.setIcon(0, icon)
                if is_col: self._set_color_icon(child, item_key)
    ...
    def _get_resize_edge(self, pos):
        ...
        if at_right and at_bottom: return 'corner'
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if handle_global_rect.contains(event.pos()):
                self._resize_edge = 'corner'; self._resize_start_pos = event.globalPos(); self._resize_start_geometry = self.geometry(); self.setCursor(Qt.SizeFDiagCursor); event.accept(); return
            
            edge = self._get_resize_edge(event.pos())
            if edge:
                self._resize_edge = edge; self._resize_start_pos = event.globalPos(); self._resize_start_geometry = self.geometry()
                if edge == 'corner': self.setCursor(Qt.SizeFDiagCursor)
            ...
            if header_global_rect.contains(event.pos()):
                self._drag_start_pos = event.pos(); self.setCursor(Qt.SizeAllCursor); event.accept(); return
        super().mousePressEvent(event)

    def mouseMoveEvent(self, event):
        if self._resize_edge and (event.buttons() & Qt.LeftButton):
            delta = event.globalPos() - self._resize_start_pos; geo = self._resize_start_geometry
            new_width = geo.width(); new_height = geo.height()
            if self._resize_edge in ['right', 'corner']: new_width = max(self.minimumWidth(), geo.width() + delta.x())
            if self._resize_edge in ['bottom', 'corner']: new_height = max(self.minimumHeight(), geo.height() + delta.y())
            self.resize(new_width, new_height); event.accept(); return
        if self._drag_start_pos and (event.buttons() & Qt.LeftButton):
            self.move(event.globalPos() - self._drag_start_pos); event.accept(); return
        event.ignore()

    def mouseReleaseEvent(self, event):
        self._drag_start_pos = None; self._resize_edge = None; self._resize_start_pos = None; self._resize_start_geometry = None; self.setCursor(Qt.ArrowCursor)
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, 'resize_handle'):
            self.resize_handle.move(self.container.width() - self.resize_handle.width() - 2, self.container.height() - self.resize_handle.height() - 2)