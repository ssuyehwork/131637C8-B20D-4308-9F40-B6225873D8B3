...
from ui.components.quick_toolbar import QuickToolbar
from ui.components.quick_sidebar import QuickSidebar
...
class QuickWindow(FramelessWindow):
    ...
    def __init__(self, db_manager):
        ...
        self._current_filter = {'type': 'all', 'value': None, 'color': None}
        ...
        self.monitor_timer.timeout.connect(self.platform_utils.monitor_foreground_window)
        ...
        # Connect signals from the new sidebar component
        self.sidebar.filter_changed.connect(self._on_filter_changed)
        self.sidebar.new_idea_requested.connect(self._do_new_idea)

        # Connect signals from the new toolbar component
        self.right_bar.close_requested.connect(self.close)
        self.right_bar.maximize_requested.connect(self.toggle_main_window_requested)
        self.right_bar.minimize_requested.connect(self.showMinimized)
        self.right_bar.pin_toggled.connect(self._toggle_stay_on_top)
        self.right_bar.toggle_sidebar_requested.connect(self._toggle_partition_panel)
        self.right_bar.prev_page_requested.connect(self._prev_page)
        self.right_bar.next_page_requested.connect(self._next_page)
        self.right_bar.page_jump_requested.connect(self._jump_to_page)
    ...
    def _init_ui(self):
        ...
        self.list_widget.startDrag = self._start_drag # Monkey-patching startDrag
        ...
        self.sidebar = QuickSidebar(self.db)
    ...
    def _do_new_idea(self, category_id=None):
        # The category_id can be passed from the sidebar's context menu
        dialog = EditDialog(self.db, category_id_for_new=category_id, parent=None)
        dialog.setAttribute(Qt.WA_DeleteOnClose)
        dialog.data_saved.connect(self._update_list)
        dialog.data_saved.connect(self.sidebar.update_sidebar)
        dialog.show()
    ...
    def _restore_window_state(self):
        ...
        self.right_bar.set_pin_status(is_pinned)
    ...
    def save_state(self):
        ...
        save_setting("quick_window_pinned", self.right_bar.btn_stay_top.isChecked())
    ...
    def showEvent(self, event):
        self.platform_utils.set_my_hwnd(int(self.winId()))
    ...
    def _jump_to_page(self, page):
        if 1 <= page <= self.total_pages:
            ...
            # If the requested page is out of bounds, just refresh the toolbar
            # to show the current valid page.
            self.right_bar.update_page_info(self.current_page, self.total_pages)
    ...
    def _on_filter_changed(self, filter_type, filter_value, color_hex):
        self.current_page = 1
        self._current_filter['type'] = filter_type
        self._current_filter['value'] = filter_value
        self._current_filter['color'] = color_hex
        self._update_list()
        self._update_partition_status_display()
    ...
    def _update_list(self):
        ...
        f_type = self._current_filter['type']
        f_val = self._current_filter['value']
        current_color = self._current_filter['color']

        # Apply dynamic list theme based on the selected category's color
    ...
    def _update_partition_status_display(self):
        if self.sidebar.isHidden():
            # This logic needs to be updated to get the current item text from the sidebar
            # For now, we'll just put a placeholder
            current_filter_text = self._current_filter['type'].capitalize()
            self.partition_status_label.setText(f"当前分区: {current_filter_text}")
    ...
    def _toggle_stay_on_top(self, is_pinned):
        self._is_pinned = is_pinned
        self.platform_utils.set_window_topmost(int(self.winId()), self._is_pinned)
    ...
    def _on_item_activated(self, item):
        ...
        try:
            ...
            self.platform_utils.paste_in_previous_window()
