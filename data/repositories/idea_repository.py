...
class IdeaRepository:
    ...
    def get_metadata_by_filter(self, search, f_type, f_val):
        ...
        """
        c = self.db.get_cursor()
        # [还原] 移除 content 字段，因为不再需要前端计算扩展名
        q = """
            SELECT 
        ...
        if search:
            # 搜索时依然可以 join content 表进行匹配，但不需要 select 出来
    ...
    def get_details_by_ids(self, id_list):
        ...
        return [res_map[iid] for iid in id_list if iid in res_map]